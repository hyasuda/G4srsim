...
void musrPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (takeMuonsFromTurtleFile) {
	...
	exit(1);
      ...
      // x      = xTmp*cm + (z0-z0_InitialTurtle)*tan(xangle) ;       // usually z0 is negative // comment out by hyasuda
      // y      = yTmp*cm + (z0-z0_InitialTurtle)*tan(yangle) ;       // z0_InitialTurtle is the z0 at whith the turtle file was generated. // comment out by hyasuda
      x = xTmp*cm ;
      y = yTmp*cm ;
      p      = pTmp*GeV;
  ...
  // add by otani
  G4cout << "before momentum calculatoin" << G4endl;
  G4cout << "xangle: " << xangle << "\t" 
	 << "yangle: " << yangle << "\t" 
	 << G4endl;
    
  G4double px, py, pz;
  // px = p*sin(xangle);
  px = p*xangle/std::sqrt(1+xangle*xangle+yangle*yangle);
  G4cout << "px: " << px << "\t"; 
  // py = p*sin(yangle);
  py = p*yangle/std::sqrt(1+xangle*xangle+yangle*yangle);
  
  G4cout << "py: " << py << "\t" ;
  // pz = std::sqrt(p*p - px*px - py*py);
  pz = p/sqrt(1+xangle*xangle+yangle*yangle);
  G4cout << "pz: " << pz << "\t" ;
    G4cout << G4endl;
  // Assign spin 
