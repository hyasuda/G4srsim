...
//tm #include "G4LowEnergyCompton.hh"
//#include "G4LowEnergyPolarizedCompton.hh"
//tm #include "G4LowEnergyGammaConversion.hh"
//tm #include "G4LowEnergyPhotoElectric.hh"
//tm #include "G4LowEnergyRayleigh.hh"
//tm #include "G4LowEnergyBremsstrahlung.hh"
//tm #include "G4LowEnergyIonisation.hh"
//tm #include "G4hLowEnergyIonisation.hh"


// For Penelope processes:
//tm #include "G4PenelopeCompton.hh"
//tm #include "G4PenelopeGammaConversion.hh"
//tm #include "G4PenelopePhotoElectric.hh"
//tm #include "G4PenelopeRayleigh.hh"
//tm #include "G4PenelopeIonisation.hh"
//tm #include "G4PenelopeBremsstrahlung.hh"
//tm #include "G4PenelopeAnnihilation.hh"

// For Coulomb scattering instead of multiple scattering
//tm #include "G4CoulombScattering.hh"
//tm #include "G4CoulombScatteringModel.hh"
...
void musrPhysicsList::ConstructEM()
{
  ...
  while (!feof(fSteeringFile)) {
	...
	if (strcmp(tmpString2,"addDiscreteProcess")==0) {
	  ...
	  */
	  ...
	  if      (stringProcessName=="G4MultipleScattering")  
	    // commented out (T. Mibe)
	    //pManager->AddProcess(new G4MultipleScattering,nr1,nr2,nr3);
	    sprintf(eMessage,"musrPhysicsList:  Process G4MultipleScattering is obsolate");
	  // changed to "e"Multiple scatteing (T. Mibe)
	  else if (stringProcessName=="G4eMultipleScattering")     pManager->AddProcess(new G4eMultipleScattering,nr1,nr2,nr3);
	  // changed to "Mu"Multiple scatteing (T. Mibe)
	  ...
	  */
	  ...
	  else if (stringProcessName=="eMultipleAndCoulombScattering") {
	  // changed to "e"Multiple scatteing (T. Mibe)
	    G4eMultipleScattering* multScat = new G4eMultipleScattering();
	    //	    G4CoulombScattering*  coulScat = new G4CoulombScattering();
	    // tm	    G4CoulombScatteringModel* coulScatModel = new G4CoulombScatteringModel();
	    /*** tm
	    if (strcmp(charRegion1,"")!=0) {
	      ...
	      // tm	      multScat->AddEmModel(0,coulScatModel,regionForCoulomb);
  ...
  while( (*theParticleIterator)() ){
    ...
    if ((particleName == "gamma")||(particleName == "e-")||(particleName == "e+")) {
      ...
      // use hadronic multiple scattering class (T. Mibe)
      pmanager->AddProcess(new G4hMultipleScattering,-1, 1,1);
      //      if (myTypeOfProcesses=="highenergy") {
      //	pmanager->AddProcess(new G4hIonisation,       -1, 2,2);
      //    }
      //    else {
      //tm      pmanager->AddProcess(new G4hLowEnergyIonisation,       -1, 2,2);
      //    }
